...
replayWithOperations machine events = (finalState, toList allOps)
  where
    s0 = initial machine
    -- First step with empty event
    StepResult s1 ops1 = step machine s0 emptyEvent
    -- Process events, collecting operations using Seq for O(1) append
    (finalState, allOps) = foldl' stepCollect (s1, Seq.singleton ops1) events
...
replayGenerate machine = foldl' stepOne (initial machine)
  where
    stepOne s e = stepState (step machine s e)
