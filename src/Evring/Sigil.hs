...
data SigilParseResult a
    = -- | Success with remaining bytes
      Ok !a !ByteString
    | -- | Need more bytes (streaming)
      Incomplete
    | -- | Protocol violation, must reset
...
data SigilState = SigilState
    { sigilParseMode :: !ParseMode
    -- ^ Lean: parseMode : ParseMode
    , sigilBuffer :: !(Seq TokenId)
    -- ^ Lean: buffer : List TokenId (using Seq for O(1) append)
    , sigilLeftover :: !ByteString
    -- ^ Lean: leftover : Bytes
    , sigilChunks :: ![Chunk]
    -- ^ Accumulated chunks (reversed)
    , sigilDone :: !Bool
...
initSigilState = SigilState ModeText Seq.empty BS.empty [] False
...
buildChunk state = Chunk content
  where
    tokens = toList (sigilBuffer state)
...
handleControlByte state opcode rest = Right $ case opcode of
    ...
    0xC0 ->
        let chunk = buildChunk state True
            newState = state{sigilBuffer = Seq.empty}
    ...
    0xC1 ->
        case sigilParseMode state of
            ModeText ->
                let pendingChunk =
                        if null (sigilBuffer state)
                            then Nothing
                            else Just (buildChunk state False)
                    newState =
                        state
                            { sigilParseMode = ModeToolCall
                            , sigilBuffer = Seq.empty
...
decodeSingleByte state byte rest
    ...
    | isHotByte byte =
        let tokenId = fromIntegral byte
            newState = state{sigilBuffer = sigilBuffer state |> tokenId}
    ...
    | isExtendedByte byte =
        case decodeVarint rest of
            ...
            Just (tokenId, consumed) ->
                let newState = state{sigilBuffer = sigilBuffer state |> tokenId}
...
processBytes !state !input !acc
    | BS.null input = (state{sigilLeftover = BS.empty}, toList acc)
    | otherwise =
        let byte = BS.head input
            rest = BS.tail input
         in case decodeSingleByte state byte rest of
                Left leftover ->
                    (state{sigilLeftover = leftover}, toList acc)
...
instance Machine SigilMachine where
    ...
    step _machine state event =
        ...
        let inputBytes = sigilLeftover state <> eventData event
            (newState, chunks) = processBytes state inputBytes Seq.empty
